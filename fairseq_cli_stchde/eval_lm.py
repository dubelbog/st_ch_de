...
import torch
import fairseq_stchde
from fairseq_stchde import checkpoint_utils, distributed_utils, options, tasks, utils
from fairseq_stchde.dataclass.utils import convert_namespace_to_omegaconf
from fairseq_stchde.logging import progress_bar
from fairseq_stchde.logging.meters import StopwatchMeter
...
def eval_lm(
    models: List[fairseq_stchde.models.FairseqModel],
    source_dictionary: fairseq_stchde.data.Dictionary,
    batch_iterator: Iterable,
    post_process: Optional[str] = None,
    output_word_probs: bool = False,
    output_word_stats: bool = False,
    target_dictionary: Optional[fairseq_stchde.data.Dictionary] = None,
    ...
    """
    Args:
        models (List[~fairseq_stchde.models.FairseqModel]): list of models to
            evaluate. Models are essentially `nn.Module` instances, but
            must be compatible with fairseq's `SequenceScorer`.
        source_dictionary (~fairseq_stchde.data.Dictionary): dictionary for
        ...
        post_process (Optional[str]): post-process text by removing BPE,
            ...
            fairseq_stchde.data.utils.post_process, although not all options
        ...
        target_dictionary (Optional[~fairseq_stchde.data.Dictionary]): output
