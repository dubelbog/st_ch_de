...
import torch
from fairseq_stchde import checkpoint_utils, models, optim, utils
...
from fairseq_stchde.logging import meters, metrics
from fairseq_stchde.nan_detector import NanDetector
from fairseq_stchde.optim import lr_scheduler
...
class Trainer(object):
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            print("Sample after preparation: ", sample)
    ...
    def get_meter(self, name):
        ...
        from fairseq_stchde import meters

        if "get_meter" not in self._warn_once:
            self._warn_once.add("get_meter")
            utils.deprecation_warning(
                "Trainer.get_meter is deprecated. Please use fairseq_stchde.metrics instead."
            )
    ...
    def _prepare_sample(self, sample, is_dummy=False):
        print("Saple befor preparation", sample)
        if sample == "DUMMY":
            raise Exception(
