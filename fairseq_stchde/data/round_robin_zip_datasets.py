...
class RoundRobinZipDatasets(FairseqDataset):
    """Zip multiple :class:`~fairseq_stchde.data.FairseqDataset` instances together.

    Shorter datasets are repeated in a round-robin fashion to match the length
    of the longest one.

    Args:
        datasets (Dict[~fairseq_stchde.data.FairseqDataset]): a dictionary of
            :class:`~fairseq_stchde.data.FairseqDataset` instances.
